...
The whole deck serves ONE sentence:
"Three students found 110 million forints lost in one department. There are 163 hospitals in Hungary."

## Type scale (only two)
--type-massive: clamp-ish, ~clamps per slide (160–280px)
--type-small: 26px (validator floor 24px; reads as a whisper against massive)
No mid sizes.

## Animation map (one per slide)
- 00: nothing. silence.
- 01: black holds 1.5s → red fades in 0.8s.
- 02: number counts 0 → 110,000,000 over 2.5s, ease-out.
- 03: SAME DATA. holds → two columns split in → "One upload." fades 0.6s.
- 04: headline holds → QR snaps in (≤0.4s).
- 05: arrows draw down one beat.
